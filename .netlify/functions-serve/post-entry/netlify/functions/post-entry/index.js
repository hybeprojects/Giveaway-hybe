...
// netlify/functions/post-entry/index.js
var index_exports = {};
...
var supabaseUrl = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || process.env.SUPABASE_DATABASE_URL || "";
var supabaseKey = process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || "";
if (!supabaseUrl || !supabaseKey) {
  throw new Error(
    "Missing Supabase configuration. Please set SUPABASE_URL and SUPABASE_ANON_KEY in your environment."
  );
}
var supabase = (0, import_supabase_js.createClient)(supabaseUrl, supabaseKey, { auth: { persistSession: false } });
...
var handler = async (event) => {
  ...
  const { data: { user }, error: authError } = await supabase_default.auth.getUser(token);
  if (authError || !user) {
    return { statusCode: 401, body: JSON.stringify({ ok: false, error: "Unauthorized" }) };
  }
  try {
    ...
    const { data: existingEntry, error: selectError } = await supabase_default.from("entries").select("email").eq("email", email).maybeSingle();
    ...
    const { error: insertError } = await supabase_default.from("entries").insert(payload);
    if (insertError) throw insertError;
    const { error: ledgerError } = await supabase_default.from("ledger_entries").insert({
      email,
      type: "credit",
      amount: welcomeBonus,
      currency: "points",
